    ...
    public class Simple_Kafka_Consumer
    {
        public static void ConsumeMessages(string bootstrapServers, string topicName, string consumerGroup = "Consumers-Group")
        {
            var config = new ConsumerConfig
            {
                ...
                GroupId = consumerGroup, // when unique Group names, auto commit=true and should start from latest
                EnableAutoCommit = true,
                AutoCommitIntervalMs = 1000,
                AutoOffsetReset = AutoOffsetReset.Latest // Start the new messages only and disregard history
